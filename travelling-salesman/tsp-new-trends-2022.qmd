...
abstract: 
  'Problém obchodního cestujícího je jedním z nejstaších a teoreticky nejlépe zpracovaných témat v oblasti optimalizačních úloh, s vysokou mírou standardizace řešení. Příspěvek ilustruje možnosti praktického využití jednozo z nástrojů - rozšiřujícího balíčku {TSP} - v kontextu statistického programovacího jazyka R. Dále demonstruje využití tří alternativních metrik pro optimalizaci: vzdálenosti vzdušnou čarou, dojezdovou dráhu autem a čas cesty autem.'
bibliography: 
...
The [Prague School of Economics](https://www.vse.cz/english/), is located in Žižkov. A formerly working class neighborhood, now rather gentrified, it has to this day retained some traces of its former rougher edges. One of these is an active night life.

A visit to the bars of Žižkov is therefore a familiar activity for many VŠE students, and can serve as a gateway drug for serious optimization techniques. Such as the Travelling Salesman Problem @gutin_punnen06.
...
We have located `r nrow(bars)` bars, implying a distance matrix of `r nrow(bars) ^2` elements. Not a huge one by today's standards – but big enough to think twice about trying to solve using a pen and a piece of paper. I have found it advantageous to be able to actually show the distance matrix – and this page will easily accommodate only about a 5×5 matrix.
...
base_plot +
  geom_sf(data = st_transform(duration_result, 3857), color = "cornflowerblue", size = 1.5) +
  geom_sf(data = st_transform(crow_result,3857), color = "goldenrod", size = 1.5) +
  geom_sf(data = st_transform(distance_result, 3857),  color = "orchid", size = 1.5) +
  geom_sf(data = st_transform(vzorek, 3857), color = "red", size = 5) +
  scale_fill_identity() +
  coord_sf() +
  theme_void()
```
